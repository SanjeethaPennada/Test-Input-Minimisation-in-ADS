...
def get_collision_rate(inp):
    ...
    try:
        ...
        if "T-bone" in output:  #Accident for weak oracle
            return 1
        elif "Safe" in output:
            return 0
        else:
...
def test_function(inp, config_log_id):
    try:
        ...
        return Outcome.FAIL if collision_result == 1 else Outcome.PASS
    except Exception as e:
...
def run_dd(dd_type):
    initial_test_input =  weather_time_conditions + npc_ids + streetlight_ids + building_ids
